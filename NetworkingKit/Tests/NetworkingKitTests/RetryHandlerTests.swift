...
struct RetryHandlerTests {
    ...
    @Test("execute with maxAttempts zero clamps to one attempt")
    func test_whenMaxAttemptsIsZero_thenMakesExactlyOneAttempt() async {
        ...
        await #expect(throws: NetworkError.self) {
            try await handler.execute(maxAttempts: 0) {
                await attempts.increment()
                throw NetworkError.timeout
            }
        }
